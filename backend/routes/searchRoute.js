...
const app=express();
mongoose.connect('mongodb+srv://node-api:1234@srh-adb-01.inyaq.mongodb.net/db01?retryWrites=true&w=majority').then(()=>{
...
router.post("/", async (req, res) => {
  ...
  try {
      ...
      if(response && response.data) {
        ...
        stockModel.QuarterlyEarningsGrowthYOY = response.data.QuarterlyEarningsGrowthYOY;
        stockModel.QuarterlyRevenueGrowthYOY = response.data.QuarterlyRevenueGrowthYOY;
...
router.get("/:selectedCompany", async (req, res) => {
  const kword = req.params.selectedCompany;
  console.log(kword);
